...
const autoKey = (mk) => `fin:auto:${mk}`                          // sortie auto: { enabled, note }
...
export default function FinancePage(){
  const [tab, setTab] = useState('perso') // 'perso' | 'pro' | 'rappels' | 'auto'
    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-2 text-sm">
        {[
          ...
          {k:'auto', label:'Sortie auto'},   // 👈 NOUVEL ONGLET
        ].map(b=>(
          <button
            key={b.k}
            onClick={()=>setTab(b.k)}
      ...
      {tab==='auto' && <AutoOutSection mk={mk} />}   {/* 👈 rendu Sortie auto */}
    </div>
  )
}
